...
function configure_postgresql() {
  ...
  pct exec $CTID --user postgres -- bash -c "psql -c \"CREATE DATABASE $POSTGRES_DB;\""
  pct exec $CTID --user postgres -- bash -c "psql -c \"CREATE USER $POSTGRES_USER WITH PASSWORD '$POSTGRES_PASSWORD';\""
  pct exec $CTID --user postgres -- bash -c "psql -c \"GRANT ALL PRIVILEGES ON DATABASE $POSTGRES_DB TO $POSTGRES_USER;\""
